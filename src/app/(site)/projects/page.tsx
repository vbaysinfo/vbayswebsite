...
export default async function ProjectsPage() {
  const { settings, projects, gallery } = await getContent();
  const imagesById = Object.fromEntries(projects.map((p) => [p.projectId, projectImages(p, gallery)]));
  return (
    <>
      ...
      <section className="section">
        <div className="container-x">
          <ProjectsFilter projects={projects} categories={GALLERY_CATEGORIES.filter((c) => c !== "Factory")} imagesById={imagesById} whatsappNumber={settings.whatsappNumber} />
        </div>
      </section>
